...
import api from "@/publicSite/lib/api";
import { isFavorite, toggleFavorite } from "@/publicSite/lib/favorites";
import { addToCart as addToCartEvt } from "@/publicSite/components/CartDrawer";
import BundleDiscountHint from "@/publicSite/components/BundleDiscountHint";
...
export default function ShopSeoCategory({ seoSlug: propSlug }) {
  ...
  useEffect(() => {
    setLoading(true);
    api.get(`/wp/products?per_page=100&page=1`)
      .then((r) => {
        const items = Array.isArray(r.data?.items) ? r.data.items : [];
        const normalized = items.map((p) => ({ ...p, images: p.images || (p.image ? [{ src: p.image, alt: p.name }] : []), categories: Array.isArray(p.categories) ? p.categories : [] }));
        const terms = {
          "concentration-bien-etre": /concentration|focus|vue|matcha|lumiere|casque/i,
          ergonomie: /ergonomie|posture|bureau|souris|support|lombaire/i,
          "pause-spirituelle": /rituel|meditation|ancrage|sens|bougie|journal/i,
          amenagement: /bureau|lampe|eclairage|ambiance|plante|organisation/i,
          "mobilite-nomade": /mobilite|nomade|voyage|sac|gourde|support/i,
          "coffrets-thematiques": /pack|coffret/i,
        };
        const matcher = terms[seoSlug] || /.*/i;
        const products = normalized.filter((p) => matcher.test(`${p.name} ${p.short_description || ""} ${p.categories.map(c => typeof c === "string" ? c : c.name).join(" ")}`));
        const labels = {
          "concentration-bien-etre": ["Concentration & bien-être", "Des outils sélectionnés pour protéger votre attention et votre confort au travail."],
          ergonomie: ["Ergonomie", "Aménagez votre poste de travail pour travailler plus confortablement au quotidien."],
          "pause-spirituelle": ["Pause & recentrage", "Des objets et rituels pour créer de vraies respirations dans votre journée."],
          amenagement: ["Aménagement du bureau", "Un environnement de travail plus fonctionnel, agréable et cohérent."],
          "mobilite-nomade": ["Mobilité nomade", "Les essentiels pour travailler efficacement hors de votre bureau habituel."],
          "coffrets-thematiques": ["Coffrets thématiques", "Des sélections pensées comme des parcours, pas comme une accumulation de produits."],
        };
        const [name, tagline] = labels[seoSlug] || ["Sélection ZAYADO", "Une sélection éditoriale pensée pour les indépendants."];
        setData({ name, tagline, count: products.length, products, editorial: EDITORIALS[seoSlug] });
      })
      .catch(() => setData(null))

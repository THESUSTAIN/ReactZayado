...
import VirtualTryOn from "@/publicSite/components/VirtualTryOn";
import Product3DViewer from "@/publicSite/components/Product3DViewer";
...
export default function BoutiqueProduct() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setActiveImg(0);
    api.get(`/wp/product/${slug}`).then((r) => {
      if (alive) setP(r.data);
    }).catch(() => {
      if (alive) setP(null);
    }).finally(() => { if (alive) setLoading(false); });
    // Charge les autres produits pour cross-sell
    api.get(`/wp/products?per_page=24&page=1`).then((r) => {
      if (alive) setCrossSell((r.data.items || []).filter((x) => x.slug !== slug).map((x) => ({ ...x, images: x.images || (x.image ? [{ src: x.image, alt: x.name }] : []) })));
    }).catch(() => {});
    ...
    api.get(`/wp/products?per_page=24&page=1`).then((r) => {
      if (!alive) return;
      const bundles = (r.data?.items || []).filter((b) =>
  ...
  const seoDescription = String(p.short_description || p.description || `${p.name} sélectionné par ZAYADO pour les indépendants et petites structures.`).replace(/<[^>]*>/g, "").slice(0, 155);
  return (
    <>
      <Helmet>
        <title>{`${p.name} | Boutique ZAYADO`}</title>
        <meta name="description" content={seoDescription} />
        <link rel="canonical" href={`https://zayado.net/boutique/${p.slug}`} />
        {img && <meta property="og:image" content={img} />}
        <meta property="og:title" content={`${p.name} | Boutique ZAYADO`} />

...
import { useWPPage, parseWPContent, getSection } from "@/publicSite/lib/wpContent";
import useWpContent from "@/publicSite/hooks/useWpContent";
import { PublicHeader, UnifiedFooter } from "@/publicSite/pages/LandingHub";
...
const Hero = ({ wp }) => {
  const hero = getSection(wp, "hero", { heading: "Le copilote IA qui transforme votre vision en <span class=\"font-serif-italic\" style=\"color:#b89855\">actions quotidiennes.</span>" });
  const titleHTML = hero.heading || "Le copilote IA qui transforme votre vision en <span class=\"font-serif-italic\" style=\"color:#b89855\">actions quotidiennes.</span>";
  const subtitleHTML = hero.subtitle || "Développez votre entreprise <b class=\"text-ink\">sans vous épuiser</b>. Pilotage, tâches et IA proactive : MyExtension AI prépare, <span class=\"fancy-underline relative\">vous décidez</span>.";
  const eyebrow = hero.extras?.eyebrow || hero.extras?.["eyebrow-petite-etiquette-en-haut"] || "Le copilote IA des entrepreneurs · by Zayado";
...
export default function MyExtensionAI() {
  ...
  const wpMeta = useWpContent("myextension-ai", {
    title: "MyExtension AI — Le copilote IA des entrepreneurs | by Zayado",
    subtitle: "MyExtension AI transforme votre vision en actions quotidiennes pour développer votre entreprise sans vous épuiser. Pilotage, tâches, IA proactive. Essai dès 1 €.",
  });
  return (
    <div className="min-h-screen bg-canvas" data-testid="myextension-page">
      <Helmet>
        ...
        <meta name="keywords" content="copilote IA entrepreneur, assistant IA business, logiciel gestion entreprise IA, outil productivité indépendant" />

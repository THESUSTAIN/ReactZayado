...
import { useWPPage, parseWPContent } from "@/publicSite/lib/wpContent";
...
export default function Contact() {
  ...
  const subjectMap = { finance: "Finance & pilotage", gestion: "Gestion & organisation", developpement: "Développement & acquisition", structuration: "Création & structuration", "finance-pilotage": "Finance & pilotage", "gestion-organisation": "Gestion & organisation" };
  const [form, setForm] = useState({ name: "", email: "", subject: subjectMap[service] || "general", message: "" });
  ...
  const heroSubtitle = wp?.subtitle || "Un besoin financier, administratif ou commercial ? Décrivez votre situation : nous vous orientons vers la bonne prochaine étape.";
        ...
        {!done ? (
          <form onSubmit={submit} className="space-y-4">
            ...
            <select value={form.subject} onChange={(e) => setForm({ ...form, subject: e.target.value })}
              ...
              <option value="Finance & pilotage">Finance & pilotage</option>
              <option value="Gestion & organisation">Gestion & organisation</option>
              <option value="Développement & acquisition">Développement & acquisition</option>
              <option value="Création & structuration">Création & structuration</option>
              <option value="commande">Suivi de commande</option>
              <option value="retour">Retour produit</option>
              <option value="partenariat">Partenariat / Presta</option>
              <option value="presse">Presse</option>
            </select>
            <textarea required rows={6} placeholder="Décrivez votre situation, votre objectif et ce qui vous bloque aujourd’hui…" value={form.message}
          ...
          <div className="p-6 rounded-xl border border-[var(--zayado-border)] bg-white text-center" data-testid="contact-success">
            ...
            <p className="text-sm" style={{ color: MUTED }}>
              Votre demande a bien été transmise à l’équipe ZAYADO.
            </p>
          </div>
        )}
        ...
        <aside className="space-y-4">
          ...
          <div className="p-5 rounded-xl" style={{ background: "var(--zayado-gold-bg)" }}>
            ...
            <p className="text-xs" style={{ color: "var(--zayado-text)" }}>
              Lun–Ven : 9h–18h<br />
              Nous vous répondons selon la nature de votre demande et le niveau d’intervention nécessaire.
            </p>
          </div>
        </aside>
      </div>

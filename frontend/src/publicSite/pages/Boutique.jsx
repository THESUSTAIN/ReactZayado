...
function Catalog({ openCart, cart, setCart, source }) {
  ...
  useEffect(() => {
    ...
    api.get(`/wp/products?${params.toString()}`)
      .then((r) => {
        if (!alive) return;
        const items = Array.isArray(r.data?.items) ? r.data.items : [];
        const normalized = items.map((p) => ({
          ...p,
          images: p.images || (p.image ? [{ src: p.image, alt: p.name }] : []),
          categories: Array.isArray(p.categories) ? p.categories : [],
          brand: p.brand || "Sélection ZAYADO",
          rating_count: Number(p.rating_count || 0),
          average_rating: Number(p.average_rating || 0),
          featured: Boolean(p.featured),
        }));
        const filtered = normalized.filter((p) => {
          const haystack = `${p.name} ${(p.categories || []).map(c => typeof c === "string" ? c : c.name).join(" ")}`.toLowerCase();
          const matchesUniverse = universe === "all" || haystack.includes(universe === "corps" ? "corps" : "âme") || (universe === "corps" && /ergonomie|vue|posture|bureau|confort/.test(haystack)) || (universe === "ame" && /sens|rituel|journal|ancrage|inspiration/.test(haystack));
          return matchesUniverse && (!search || haystack.includes(search.toLowerCase()));
        });
        const sorted = [...filtered].sort((a,b) => sort === "price-asc" ? Number(a.price||0)-Number(b.price||0) : sort === "price-desc" ? Number(b.price||0)-Number(a.price||0) : sort === "newest" ? Number(b.id||0)-Number(a.id||0) : 0);
        setProducts(sorted);
      })
  ...
  const showSourceBanner = source !== "woocommerce" && products.length === 0;

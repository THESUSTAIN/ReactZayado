  ...
  return (
    <div className="space-y-6" data-testid="page-campus">
      <section className="glass relative overflow-hidden p-6 sm:p-8">
        <div className="absolute -right-16 -top-20 h-48 w-48 rounded-full bg-[#D4AF37]/10 blur-3xl" aria-hidden="true" />
        <div className="relative max-w-3xl">
          <div className="flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[.16em] text-[#D4AF37]"><GraduationCap size={15} /> {copy.eyebrow}</div>
      ...
      {loading ? (
        ...
        <section className="glass p-6 sm:p-8" data-testid="campus-login-state">
          <p className="text-[11px] font-semibold uppercase tracking-[.16em] text-[#D4AF37]">Accès personnel</p>
        ...
        <section className="grid grid-cols-1 gap-4 lg:grid-cols-[1.1fr_.9fr]">
          <form onSubmit={start} className="glass p-6" data-testid="campus-start-form">
            <p className="text-[11px] font-semibold uppercase tracking-[.16em] text-[#D4AF37]">Première configuration</p>
            ...
            <div className="mt-5 grid grid-cols-1 gap-2 sm:grid-cols-3">
              {PARCOURS.map((item) => <button key={item.id} type="button" onClick={() => setTrack(item.id)} className={`rounded-xl border p-3 text-left transition-colors ${track === item.id ? "border-[#D4AF37]/60 bg-[#D4AF37]/10" : "border-white/15 bg-white/[.03] hover:bg-white/[.06]"}`}><span className="block text-sm font-semibold text-white">{item.label}</span><span className="mt-1 block text-xs leading-relaxed text-white/50">{item.detail}</span></button>)}
            </div>
            <label className="mt-5 block text-xs font-medium text-white/65">Métier, domaine ou formation visé(e)</label>
            <input value={programme} onChange={(event) => setProgramme(event.target.value)} placeholder="Ex. marketing digital, gestion de projet, relation client…" className="mt-1.5 w-full rounded-xl border border-white/15 bg-white/[.05] px-3.5 py-2.5 text-sm text-white placeholder:text-white/35 outline-none focus:border-[#D4AF37]/60" />
            <label className="mt-4 block text-xs font-medium text-white/65">Niveau de départ</label>
            <select value={diploma} onChange={(event) => setDiploma(event.target.value)} className="mt-1.5 w-full rounded-xl border border-white/15 bg-[#102554] px-3.5 py-2.5 text-sm text-white outline-none focus:border-[#D4AF37]/60">{DIPLOMES.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select>
          ...
          <aside className="glass p-6">
            <p className="text-[11px] font-semibold uppercase tracking-[.16em] text-[#D4AF37]">Ce qui vous attend</p>

...
export default function CampusComingSoon() {
  ...
  return (
    <div className="space-y-6" data-testid="campus-coming-soon">
      <header>
        <p className="text-[11px] font-semibold uppercase tracking-[.16em] text-[#D4AF37]">MyExtension Campus</p>
      ...
      <nav className="flex gap-2 overflow-x-auto pb-1" aria-label="Navigation Campus">
        {TABS.map(([path, label], index) => <button key={path} type="button" onClick={() => navigate(path)} className={`whitespace-nowrap rounded-full border px-3.5 py-2 text-sm font-semibold ${index === 0 ? "border-[#D4AF37]/40 bg-[#D4AF37] text-[#0A1128]" : "border-white/15 bg-white/[.04] text-white/55"}`}>{label}</button>)}
      </nav>
      <section className="glass flex min-h-[390px] items-center justify-center p-8 text-center sm:p-12">
        <div className="max-w-xl">
          <span className="mx-auto flex h-16 w-16 items-center justify-center rounded-2xl border border-[#D4AF37]/30 bg-[#D4AF37]/10 text-[#E5C887]"><GraduationCap size={30} /></span>
          <p className="mt-6 text-[11px] font-semibold uppercase tracking-[.18em] text-[#D4AF37]">Campus</p>
          ...
          <button type="button" disabled={!isProjectAdmin} onClick={() => isProjectAdmin && navigate("/campus/missions")} className="mt-5 inline-flex items-center gap-2 rounded-xl border border-[#D4AF37]/25 px-4 py-2.5 text-sm font-semibold text-[#E5C887] disabled:cursor-not-allowed disabled:opacity-45" title="Réservé à l’administrateur du projet"><ShieldCheck size={15} /> Ouvrir — admin du projet uniquement</button>
        </div>
      </section>
    </div>
  );

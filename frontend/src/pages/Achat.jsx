...
export default function Achat() {
  ...
  return <main className="min-h-screen px-4 py-10 text-offwhite sm:px-8"><div className="mx-auto max-w-lg">
    ...
    <nav className="-mt-5 mb-8 flex flex-wrap gap-3 text-xs text-offwhite/55" data-testid="achat-nav">
      ...
      <Link to="/parametres#facturation" className="hover:text-offwhite">Mes paiements</Link>
    </nav>
    <div className="rounded-3xl border border-white/10 bg-white/[0.04] p-6 sm:p-8"><p className="text-[10px] uppercase tracking-[0.25em] text-gold">Paiement sécurisé Mollie</p>
      ...
      {!offerId && <div className="mt-5 space-y-3 text-sm text-offwhite/70" data-testid="achat-sans-offre">
        <p>Cette page sert à régler un produit ou un service précis : on y arrive depuis un lien de paiement.</p>
        <div className="flex flex-wrap gap-2">
          <Link to="/pricing" className="rounded-xl bg-gold px-4 py-2 text-xs font-semibold text-navy-900">Voir les offres Zayado</Link>
          <Link to="/app" className="rounded-xl border border-white/15 px-4 py-2 text-xs font-semibold text-offwhite">Retour au cockpit</Link>
        </div>
      </div>}

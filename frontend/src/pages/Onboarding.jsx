...
import { Loader2, ArrowRight, ArrowLeft, Check, Compass, Sparkles, Flag, Target, RefreshCw } from "lucide-react";
import { toast } from "sonner";
import { authOnboarding, structureVision, applyVisionStructure } from "../lib/api";
...
export default function Onboarding() {
  ...
  // Étape « l'IA prépare, vous décidez » : la proposition est affichée, jamais
  // appliquée d'office. Tant que l'utilisateur n'a pas validé, rien n'est écrit.
  const [proposal, setProposal] = useState(null);
  const [structuring, setStructuring] = useState(false);
  const [structureUnavailable, setStructureUnavailable] = useState(null);
  ...
  const submit = async () => {
    ...
    try {
      ...
      setDone(true);
      if (res?.vision_saved) {
        toast.success("Votre Vision est enregistrée.");
        askStructure(vision.trim());
      }
    } catch {
  ...
  const askStructure = async (visionText) => {
    if (!visionText) return;
    setStructuring(true);
    setStructureUnavailable(null);
    try {
      const res = await structureVision(visionText);
      // Si l'IA n'est pas disponible, on le DIT — on ne fabrique pas une
      // structure générique qui se ferait passer pour une analyse.
      if (res?.available && res.proposal) setProposal(res.proposal);
      else setStructureUnavailable(res?.reason || "Le service IA n'est pas disponible.");
    } catch {
      setStructureUnavailable("Le service IA n'a pas répondu.");
    } finally {
      setStructuring(false);
    }
  };

  const acceptStructure = async () => {
    setApplying(true);
    try {
      await applyVisionStructure(proposal);
      toast.success("Votre cockpit est configuré.");
      navigate("/");
    } catch {
      toast.error("Impossible d'enregistrer cette structure.");
      setApplying(false);
    }
  };
  ...
  if (done) {
    return (
      <div className="onboarding-screen min-h-screen p-4" data-testid="page-onboarding">
        <div className="mx-auto max-w-lg py-14">
          ...
          {structuring && (
            <div className="glass mt-5 flex items-center gap-3 rounded-2xl p-5 text-sm text-white/70" data-testid="onboarding-structuring">
              <Loader2 size={17} className="animate-spin text-[#DEC2A3]" />
              L'IA lit votre Vision et prépare une structure de départ…
            </div>
          )}

          {proposal && (
            <div className="mt-6" data-testid="onboarding-proposal">
              <p className="inline-flex items-center gap-2 text-[11px] font-bold uppercase tracking-[.16em] text-[#DEC2A3]">
                <Sparkles size={13} /> L'IA propose · vous décidez
              </p>
              <p className="mt-2 text-sm leading-relaxed text-white/60">
                Voici un point de départ, pas une vérité. Validez pour l'installer dans votre cockpit,
                ou ignorez et construisez-le vous-même.
              </p>

              <div className="glass mt-4 rounded-2xl p-5">
                <p className="text-[11px] font-semibold uppercase tracking-[.12em] text-white/60">Vos 3 axes</p>
                <div className="mt-3 space-y-3">
                  {proposal.pillars.map((pillar, index) => (
                    <div key={index} className="flex gap-3">
                      <span className="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-lg bg-[#DEC2A3]/15 text-[11px] font-bold text-[#F0DCA5]">
                        {index + 1}
                      </span>
                      <div className="min-w-0">
                        <p className="text-sm font-semibold text-white">{pillar.name}</p>
                        {pillar.why && <p className="mt-0.5 text-xs leading-relaxed text-white/50">{pillar.why}</p>}
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="mt-5 border-t border-white/10 pt-4">
                  <p className="inline-flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-[.12em] text-white/60">
                    <Flag size={12} /> Premier jalon
                  </p>
                  <p className="mt-1.5 text-sm font-semibold text-white">{proposal.milestone.title}</p>
                  {proposal.milestone.evidence && (
                    <p className="mt-0.5 text-xs leading-relaxed text-white/50">Preuve : {proposal.milestone.evidence}</p>
                  )}
                </div>

                <div className="mt-4 border-t border-white/10 pt-4">
                  <p className="inline-flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-[.12em] text-white/60">
                    <Target size={12} /> Votre première action
                  </p>
                  <p className="mt-1.5 text-sm font-semibold text-white">{proposal.first_action.title}</p>
                  {proposal.first_action.why_now && (
                    <p className="mt-0.5 text-xs leading-relaxed text-white/50">{proposal.first_action.why_now}</p>
                  )}
                </div>
              </div>

              <button onClick={acceptStructure} disabled={applying} data-testid="onboarding-accept-structure"
                className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl bg-[#DEC2A3] py-3 text-sm font-semibold text-[#0A1128] hover:opacity-90 disabled:opacity-60">
                {applying ? <Loader2 size={15} className="animate-spin" /> : <>Installer dans mon cockpit <ArrowRight size={15} /></>}
              </button>
              <div className="mt-3 flex flex-wrap items-center justify-between gap-x-4 gap-y-2">
                <button onClick={() => askStructure(vision.trim())} disabled={structuring}
                  className="inline-flex items-center gap-1.5 text-xs text-white/50 hover:text-white/85" data-testid="onboarding-retry-structure">
                  <RefreshCw size={12} /> Une autre proposition
                </button>
                <button onClick={() => navigate("/")} className="text-xs text-white/62 hover:text-white/75" data-testid="onboarding-skip-structure">
                  Non merci, je construis moi-même
                </button>
              </div>
            </div>
          )}
          ...
          {structureUnavailable && (
            <div className="glass mt-5 rounded-2xl p-5" data-testid="onboarding-structure-unavailable">
              <p className="text-sm text-white/70">
                Je n'ai pas pu préparer de structure automatiquement : {structureUnavailable}
              </p>
              <p className="mt-1.5 text-xs leading-relaxed text-white/62">
                Votre Vision est bien enregistrée — vous pouvez construire vos axes à la main dans la page Vision.
              </p>
            </div>
          )}
        ...
        {step === 1 ? (
          ...
          <>
            ...
            <div className="glass mt-6 rounded-2xl p-5">
              ...
              <p className="mt-3 text-[11px] font-semibold uppercase tracking-[.12em] text-white/60">Des exemples, pour démarrer</p>
            ...
            <div className="mt-3 flex items-center justify-between">
              ...
              <button onClick={submit} disabled={saving} data-testid="onboarding-skip-vision"
                className="text-xs text-white/62 hover:text-white/75">

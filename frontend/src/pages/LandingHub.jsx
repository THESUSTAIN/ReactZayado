  ...
  return (
    <footer className="bg-[#102945] text-white">
      <div className="max-w-7xl mx-auto px-4 md:px-8 py-10 grid md:grid-cols-4 gap-8">
        ...
        <div><div className="text-xs uppercase tracking-widest text-white/40">MyExtension AI</div><div className="mt-3 space-y-2 text-sm"><Link to="/myextension-ai" className="block text-white/75">Le cockpit</Link><Link to="/myextension-ai/vision" className="block text-white/75">Vision</Link><Link to="/myextension-ai/pilotage-financier" className="block text-white/75">Pilotage financier</Link><Link to="/myextension-ai/espace-travail" className="block text-white/75">Espace de travail</Link><Link to="/myextension-ai/energie" className="block text-white/75">Énergie</Link><Link to="/tarifs" className="block text-white/75">Tarifs</Link></div></div>
    ...
    <div className="min-h-screen overflow-x-hidden" style={{ background: `linear-gradient(180deg, ${CREAM} 0%, #fffdfa 100%)`, color: INK }}>
      ...
      <section className="max-w-7xl mx-auto px-4 md:px-8 pb-24">
        <div className="rounded-[34px] bg-[#102945] text-white p-8 md:p-12 grid md:grid-cols-2 gap-10 items-center">
          <div><div className="text-xs uppercase tracking-[.22em] text-white/45">Le cockpit</div><h2 className="mt-3 text-4xl md:text-5xl font-serif">MyExtension AI relie tout ce que vous faites.</h2><p className="mt-5 text-white/65 max-w-xl">Vision, pilotage financier, travail, CRM et énergie réunis dans une même expérience.</p><Link to="/myextension-ai" className="mt-7 inline-flex items-center gap-2 rounded-full bg-white text-[#102945] px-5 py-3 text-sm font-semibold">Voir MyExtension AI <ArrowRight size={16}/></Link></div>
          <div className="grid grid-cols-2 gap-4">{[['Vision','Votre trajectoire'],['Finance','Vos chiffres'],['Travail','Vos missions'],['CRM','Vos relations'],['Énergie','Votre rythme']].map(([a,b])=><div key={a} className="rounded-2xl border border-white/10 bg-white/5 p-4"><div className="font-semibold">{a}</div><div className="text-xs text-white/50 mt-1">{b}</div></div>)}</div>
        </div>
      </section>

...
export function StrategicDecisions() {
  ...
  const approve = async (decision) => {
    try {
      await applyStrategicDecision(decision.id);
      await load();
      // Corrige le point 9 : un toast furtif ne suffit pas pour une action
      // qui crée réellement une mission — un vrai accusé reste affiché
      // directement dans la carte, pas seulement une notification qui
      // disparaît en quelques secondes.
      setJustApprovedId(decision.id);
      toast.success("Mission créée dans Mon Mouvement.");
    }

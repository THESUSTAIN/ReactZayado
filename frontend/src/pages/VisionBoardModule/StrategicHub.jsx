    ...
    <section className="glass mb-5 overflow-hidden p-5 md:p-6" data-testid="strategic-cap-home">
      <div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-start">
        <div className="max-w-3xl">
          <span className="mb-2 inline-flex items-center gap-2 text-[10px] font-bold tracking-[.16em] text-[#E5C887]"><Compass size={13} /> MON CAP VIVANT</span>
      ...
      <div className="mt-5 grid gap-3 lg:grid-cols-[1.15fr_.85fr_.85fr]">
        <article className="rounded-2xl border border-white/20 bg-white/[0.08] p-4">
          <div className="flex items-center justify-between gap-3"><span className="text-[10px] font-bold tracking-[.14em] text-[#E5C887]">LE PROCHAIN JALON</span><Flag size={15} className="text-[#E5C887]" /></div>
          {loading ? <div className="mt-5 flex items-center gap-2 text-sm text-white/55"><Loader2 size={15} className="animate-spin" /> Lecture de la trajectoire…</div> : now[0] ? <><h3 className="mt-3 text-base font-semibold text-white">{now[0].title}</h3><p className="mt-1 text-sm text-white/65">{now[0].expected_evidence || "Preuve à préciser"}</p><button onClick={onOpenHorizon} className="mt-4 inline-flex items-center gap-1 text-xs font-semibold text-[#E5C887]">Voir l’Horizon 90 jours <ArrowRight size={13} /></button></> : <><p className="mt-3 text-sm text-white/65">Aucun jalon actif. Créez la prochaine étape qui rendra votre Cap concret.</p><button onClick={onOpenHorizon} className="mt-4 inline-flex items-center gap-1 text-xs font-semibold text-[#E5C887]">Créer un jalon <Plus size={13} /></button></>}
        </article>
        <article className="rounded-2xl border border-white/20 bg-white/[0.08] p-4">
          <span className="text-[10px] font-bold tracking-[.14em] text-[#E5C887]">DÉCISION PRIORITAIRE</span>
          {pending ? <><h3 className="mt-3 text-base font-semibold text-white">{pending.title}</h3><p className="mt-1 line-clamp-2 text-sm text-white/65">{pending.why_now || "Raisonnement à préciser."}</p><button onClick={onOpenDecisions} className="mt-4 inline-flex items-center gap-1 text-xs font-semibold text-[#E5C887]">Décider maintenant <ArrowRight size={13} /></button></> : <><p className="mt-3 text-sm text-white/65">Aucune décision stratégique en attente.</p><button onClick={onOpenDecisions} className="mt-4 inline-flex items-center gap-1 text-xs font-semibold text-[#E5C887]">Préparer une décision <Plus size={13} /></button></>}
        </article>
        <article className="rounded-2xl border border-white/20 bg-white/[0.08] p-4">
          <span className="text-[10px] font-bold tracking-[.14em] text-[#E5C887]">PREUVES D’EXÉCUTION</span>
          ...
          <button onClick={onOpenHorizon} className="mt-4 inline-flex items-center gap-1 text-xs font-semibold text-[#E5C887]">Relier ma trajectoire <ArrowRight size={13} /></button>
        </article>
      </div>
    </section>
  );
...
export function StrategicHorizon() {
  ...
  return <section className="space-y-4" data-testid="strategic-horizon">
    <div className="glass p-5"><span className="text-[10px] font-bold tracking-[.16em] text-[#E5C887]">HORIZON 90 JOURS</span><h2 className="mt-2 font-head text-2xl font-semibold text-white">Rendre le Cap visible sans transformer chaque idée en urgence.</h2><p className="mt-1 max-w-3xl text-sm text-white/65">Un jalon est une preuve attendue ou une étape stratégique ; une mission ne sera créée qu’après une décision validée.</p></div>
    <form onSubmit={create} className="glass grid gap-3 p-4 md:grid-cols-2 xl:grid-cols-5">
      <input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder="Prochain jalon stratégique" className="rounded-xl border border-white/25 bg-white/[0.08] px-3 py-2.5 text-sm text-white outline-none placeholder:text-white/45 focus:border-[#E5C887] xl:col-span-2" />
      <input value={form.pillar_id} onChange={(e) => setForm({ ...form, pillar_id: e.target.value })} placeholder="Axe ou pilier (optionnel)" className="rounded-xl border border-white/25 bg-white/[0.08] px-3 py-2.5 text-sm text-white outline-none placeholder:text-white/45 focus:border-[#E5C887]" />
      <select value={form.time_window} onChange={(e) => setForm({ ...form, time_window: e.target.value })} className="rounded-xl border border-white/25 bg-[#172C5C] px-3 py-2.5 text-sm text-white outline-none focus:border-[#E5C887]"><option value="now">Maintenant</option><option value="next">Ensuite</option><option value="later">Plus tard</option></select>
      <select value={form.project_id} onChange={(e) => setForm({ ...form, project_id: e.target.value })} className="rounded-xl border border-white/25 bg-[#172C5C] px-3 py-2.5 text-sm text-white outline-none focus:border-[#E5C887]"><option value="">Projet lié (optionnel)</option>{projects.map((project) => <option key={project.id} value={project.id}>{project.nom || project.name || project.title}</option>)}</select>
      <input value={form.expected_evidence} onChange={(e) => setForm({ ...form, expected_evidence: e.target.value })} placeholder="Quelle preuve attendue ?" className="rounded-xl border border-white/25 bg-white/[0.08] px-3 py-2.5 text-sm text-white outline-none placeholder:text-white/45 focus:border-[#E5C887] md:col-span-2 xl:col-span-4" />
    ...
    <div className="grid gap-4 xl:grid-cols-3">{WINDOWS.map((window) => <section key={window.id} className="glass min-h-64 p-4"><div className="flex items-start justify-between gap-3"><div><span className="text-[10px] font-bold tracking-[.14em] text-[#E5C887]">{window.label.toUpperCase()}</span><p className="mt-1 text-xs leading-5 text-white/55">{window.hint}</p></div><TimerReset size={15} className="text-white/50" /></div><div className="mt-4 space-y-2">{milestones.filter((item) => item.time_window === window.id).map((item) => <article key={item.id} className="rounded-xl border border-white/20 bg-white/[0.08] p-3"><div className="flex gap-2"><CircleDot size={14} className="mt-0.5 shrink-0 text-[#E5C887]" /><div><strong className="text-sm text-white">{item.title}</strong><p className="mt-1 text-xs text-white/60">{item.expected_evidence || "Preuve à préciser"}</p><span className="mt-2 inline-block text-[10px] font-semibold text-[#E5C887]">{strategicStatus(item.status)}</span></div></div></article>)}{!milestones.some((item) => item.time_window === window.id) && <p className="rounded-xl border border-dashed border-white/20 p-3 text-xs leading-5 text-white/50">Aucun jalon ici. Cette fenêtre reste volontairement vide tant que vous ne l’avez pas choisi.</p>}</div></section>)}</div>
  </section>;
}
...
export function StrategicDecisions() {
  ...
  return <section className="space-y-4" data-testid="strategic-decisions">
    <div className="glass p-5"><span className="text-[10px] font-bold tracking-[.16em] text-[#E5C887]">DÉCISIONS & VALIDATION</span><h2 className="mt-2 font-head text-2xl font-semibold text-white">Une décision doit être expliquée, choisie puis traçable.</h2><p className="mt-1 max-w-3xl text-sm text-white/65">Valider crée une mission dans Mon Mouvement. Reporter conserve la décision dans votre journal, sans effacer le raisonnement.</p></div>
    <form onSubmit={create} className="glass grid gap-3 p-4 md:grid-cols-2"><input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder="Décision à arbitrer" className="rounded-xl border border-white/25 bg-white/[0.08] px-3 py-2.5 text-sm text-white outline-none placeholder:text-white/45 focus:border-[#E5C887]" /><select value={form.milestone_id} onChange={(e) => setForm({ ...form, milestone_id: e.target.value })} className="rounded-xl border border-white/25 bg-[#172C5C] px-3 py-2.5 text-sm text-white outline-none focus:border-[#E5C887]"><option value="">Jalon lié (optionnel)</option>{milestones.map((item) => <option key={item.id} value={item.id}>{item.title}</option>)}</select><textarea value={form.why_now} onChange={(e) => setForm({ ...form, why_now: e.target.value })} placeholder="Pourquoi cette décision maintenant ?" rows={3} className="rounded-xl border border-white/25 bg-white/[0.08] px-3 py-2.5 text-sm text-white outline-none placeholder:text-white/45 focus:border-[#E5C887]" /><textarea value={form.impact} onChange={(e) => setForm({ ...form, impact: e.target.value })} placeholder="Quel résultat ou quel coût du report ?" rows={3} className="rounded-xl border border-white/25 bg-white/[0.08] px-3 py-2.5 text-sm text-white outline-none placeholder:text-white/45 focus:border-[#E5C887]" /><textarea value={form.detail} onChange={(e) => setForm({ ...form, detail: e.target.value })} placeholder="Contexte de décision (optionnel)" rows={2} className="rounded-xl border border-white/25 bg-white/[0.08] px-3 py-2.5 text-sm text-white outline-none placeholder:text-white/45 focus:border-[#E5C887] md:col-span-2" /><button disabled={saving} className="gold-bg inline-flex items-center justify-center gap-2 rounded-xl px-4 py-2.5 text-sm font-semibold text-[#0B1F3A] md:col-span-2 disabled:opacity-60">{saving ? <Loader2 size={15} className="animate-spin" /> : <Sparkles size={15} />} Préparer la décision</button></form>
    <div className="space-y-3">{decisions.map((decision) => <article key={decision.id} className="glass p-4"><div className="flex flex-col justify-between gap-4 md:flex-row"><div className="max-w-3xl"><span className="text-[10px] font-bold tracking-[.13em] text-[#E5C887]">{(decision.status || "pending").toUpperCase()}</span><h3 className="mt-1 text-base font-semibold text-white">{decision.title}</h3>{decision.why_now && <p className="mt-2 text-sm text-white/75"><b className="font-semibold text-[#E5C887]">Pourquoi maintenant :</b> {decision.why_now}</p>}{decision.impact && <p className="mt-1 text-sm text-white/65"><b className="font-semibold text-white/85">Impact :</b> {decision.impact}</p>}{decision.detail && <p className="mt-2 text-sm text-white/55">{decision.detail}</p>}</div>{decision.status === "pending" && <div className="flex shrink-0 flex-wrap items-start gap-2"><button onClick={() => approve(decision)} className="gold-bg inline-flex items-center gap-2 rounded-xl px-3 py-2 text-xs font-bold text-[#0B1F3A]"><CheckCircle2 size={14} /> Créer la mission</button><button onClick={() => defer(decision)} className="rounded-xl border border-white/25 px-3 py-2 text-xs font-semibold text-white/80 hover:bg-white/10">Reporter</button></div>}</div></article>)}{!decisions.length && <div className="glass p-6 text-center"><TrendingUp size={20} className="mx-auto text-[#E5C887]" /><p className="mt-3 text-sm text-white/65">Aucune décision stratégique en attente. Créez-en une lorsque l’arbitrage mérite une trace et une mission.</p></div>}</div>
  </section>;
}

...
import {
  ...
  getTresorerieHistory, getDecision, simulatePilotage, getHealthScore, exportCsvUrl,
} from "../lib/api";
import {
...
export default function Pilotage() {
  ...
  const load = async () => {
    ...
    setDepenses(await getDepenses());
    setHistory(await getTresorerieHistory().catch(() => []));
    setDecision(await getDecision().catch(() => null));
    setScore(await getHealthScore().catch(() => null));
  };
  useEffect(() => { load(); }, []);
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="font-head text-3xl sm:text-4xl font-semibold flex items-center gap-2">
            <span className="gold-text">Pilotage</span>
            <span className="text-white/40 text-lg font-normal">(DAF IA)</span>
          </h1>
          <p className="text-white/55 text-sm mt-1">Suivez vos indicateurs clés, anticipez et prenez les bonnes décisions.</p>
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
        ...
        <div className="glass p-5">
          <h3 className="font-head font-semibold flex items-center gap-2 mb-3">
            <span className="text-[#D4AF37]">✦</span> Analyse DAF IA
          </h3>

...
import { ArrowRight, ArrowLeft, Sparkles, Check, Loader2, Compass, Palette, PanelLeft, PanelBottom, Sun, Moon, Info, BookOpen, Bell, Download } from "lucide-react";
import { usePrefs } from "@/context/PrefsContext";
import { onboardingApi, wellnessApi } from "@/lib/api";
...
export default function Onboarding({ onDone, replay = false }) {
  ...
  const [form, setForm] = useState({
    prenom: "", entreprise: "", statut: "", secteur: "", projet_description: "", pourquoi: "", objectif_90j: "",
    priorites: [], ambiance: "sens", workspace_type: "none", workspace_url: "", rappel_cadence: "quotidien", energie: 3,
  });
  ...
  const doSave = async () => {
    setSaving(true);
    try {
      const r = await onboardingApi.save({ ...form, why: form.pourquoi, spirituality: form.ambiance, project_type: detectedType });
      setResult(r);
    } catch {
  ...
  const finish = () => {
    setPref({ onboarded: true, ambiance: form.ambiance, first_name: form.prenom, show_inspiration_screen: true, vision_reminder: form.rappel_cadence });
    // Univers Foi : on amorce le parcours d'habitudes chrétiennes (best-effort).
    if (form.ambiance === "foi") { wellnessApi.seedChristianHabits().catch(() => {}); }
    onDone && onDone();
  };

  // One-Pager Vision généré à partir des réponses (chapitre "1er document").
  const visionDocText = `Pourquoi : ${form.pourquoi || "—"}\n\nProjet : ${form.projet_description || "—"}\n\nObjectif 90 jours : ${form.objectif_90j || "—"}\n\nSecteur : ${form.secteur || "—"}\n\nFreins à lever : ${form.priorites.join(", ") || "—"}`;
  const downloadVisionDoc = () => {
    try {
      const blob = new Blob([`ONE-PAGER VISION — ${form.prenom}\n(MyExtension AI · by Zayado)\n\n${visionDocText}`], { type: "text/plain;charset=utf-8" });
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url; a.download = `vision-${(form.prenom || "moi").toLowerCase()}.txt`;
  ...
  const stepMeta = {
    ...
    5: { tag: "VOTRE VISION", n: 2 },
    7: { tag: "VOS PRIORITÉS", n: 3 },
    8: { tag: "VOTRE INSPIRATION", n: 4 },
    12: { tag: "ANCRAGE", n: 6 },
    13: { tag: "BIEN-ÊTRE", n: 6 },
    9: { tag: "VOS OUTILS", n: 5 },
    10: { tag: "SYNTHÈSE", n: 6 },
    14: { tag: "PASSAGE À L'ACTION", n: 6 },
    15: { tag: "PREMIER DOCUMENT", n: 6 },
    ...
    <div className="ob-overlay" data-testid="onboarding-overlay">
      ...
      <AnimatePresence mode="wait">
        <motion.div key={screen} className="ob-modal" {...fade}>
          ...
          {screen === 5 && (
            <>
              <h2 className="ob-title" data-testid="ob-step-title">Votre vision</h2>
              <p className="ob-sub">Commençons par le sens : votre pourquoi, puis votre projet en 2 phrases.</p>
              <Field label="Votre pourquoi profond — pourquoi ce projet compte pour vous ?">
                <textarea className="ob-input ob-textarea" rows={2} value={form.pourquoi} data-testid="ob-pourquoi"
                  onChange={(e) => set({ pourquoi: e.target.value })}
                  placeholder="Ex : offrir une vie meilleure à ma famille, transmettre un savoir-faire, avoir un impact utile…" />
              </Field>
          ...
          {screen === 8 && (
            <>
              <h2 className="ob-title" data-testid="ob-step-title">Votre source d'inspiration</h2>
              <p className="ob-sub">Chaque jour, une pensée pour garder le cap. Deux univers : l'inspiration universelle (pour tous) — ou la Foi, si elle vous porte.</p>
              <div className="ob-ambiance">
                <button type="button" data-testid="ob-ambiance-sens"
                  ...
                  <span className="ob-ambiance-name"><Compass size={15} /> Sagesse & Sens</span>
                  <span className="ob-ambiance-quote">« {CITATIONS_SENS[0]} »</span>
                  <span className="ob-ambiance-desc">Pour tous — inspiration universelle, sagesse et sens.</span>
                </button>
                <button type="button" data-testid="ob-ambiance-foi"
                  className={`ob-ambiance-card ${form.ambiance === "foi" ? "on" : ""}`}
                  onClick={() => set({ ambiance: "foi" })}>
                  <div aria-hidden="true" style={{
                    height: 46, borderRadius: 8, marginBottom: 10, overflow: "hidden",
                    background: "linear-gradient(135deg, #0B1F3A 0%, #1a2a1f 100%)",
                    border: "1px solid rgba(201,164,73,0.35)", position: "relative",
                  }}>
                    <div style={{ position: "absolute", top: 8, left: 8, right: 8, height: 4, borderRadius: 2, background: "#C9A449" }} />
                    <div style={{ position: "absolute", top: 18, left: 8, width: "58%", height: 4, borderRadius: 2, background: "rgba(255,255,255,0.5)" }} />
                    <div style={{ position: "absolute", top: 28, left: 8, width: "38%", height: 4, borderRadius: 2, background: "rgba(255,255,255,0.28)" }} />
                  </div>
                  <span className="ob-ambiance-name" style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
                    <BookOpen size={15} /> Univers Foi
                    <a href="https://thesustain.net/bible" target="_blank" rel="noreferrer"
                      onClick={(e) => e.stopPropagation()} data-testid="ob-foi-bible-link"
                      title="La Bible sur TheSustain" aria-label="En savoir plus — la Bible sur TheSustain"
                      style={{ display: "inline-flex", color: "#C9A449" }}>
                      <Info size={13} />
                    </a>
                  </span>
                  <span className="ob-ambiance-quote">« {CITATIONS_FOI[0]} »</span>
                  <span className="ob-ambiance-desc">Versets bibliques, sans commentaire · approfondir sur TheSustain.</span>
                </button>
              </div>
              <p style={{ fontSize: 11.5, color: "var(--muted)", lineHeight: 1.5, margin: "12px auto 0", maxWidth: 420, textAlign: "center" }}>
                Quel que soit votre choix, l'inspiration reste bienveillante et jamais imposée — la sagesse et la nature nous enseignent, en douceur.
              </p>
              <Nav onBack={() => setScreen(7)} onNext={() => setScreen(12)} />
          ...
          {screen === 9 && (
            <>
              ...
              <Nav onBack={() => setScreen(13)} onNext={() => setScreen(10)} nextLabel="Voir la synthèse" />
          ...
          {/* Chapitre — Ancrage de la vision */}
          {screen === 12 && (
            <>
              <h2 className="ob-title" data-testid="ob-step-title">Ancrez votre vision</h2>
              <p className="ob-sub">Une vision qu'on ne relit jamais s'oublie. À quelle fréquence voulez-vous un rappel de votre pourquoi ?</p>
              <div className="ob-ambiance">
                {[["quotidien", "Chaque matin", "Un rappel doux au réveil"], ["hebdo", "Chaque semaine", "Un point vision hebdomadaire"], ["aucun", "Pas de rappel", "Je m'en occupe moi-même"]].map(([val, name, desc]) => (
                  <button key={val} type="button" data-testid={`ob-ancrage-${val}`}
                    className={`ob-ambiance-card ${form.rappel_cadence === val ? "on" : ""}`}
                    onClick={() => set({ rappel_cadence: val })}>
                    <span className="ob-ambiance-name"><Bell size={15} /> {name}</span>
                    <span className="ob-ambiance-desc">{desc}</span>
                  </button>
                ))}
              </div>
              <Nav onBack={() => setScreen(8)} onNext={() => setScreen(13)} />
            </>
          )}
          ...
          {screen === 13 && (
            <>
              <h2 className="ob-title" data-testid="ob-step-title">Et vous, comment allez-vous ?</h2>
              <p className="ob-sub">Développer sans s'épuiser commence ici. Votre énergie du moment, sur 5 ?</p>
              <div style={{ display: "flex", gap: 10, justifyContent: "center", margin: "8px 0 4px" }}>
                {[1, 2, 3, 4, 5].map((n) => (
                  <button key={n} type="button" data-testid={`ob-energie-${n}`}
                    onClick={() => set({ energie: n })}
                    style={{
                      width: 52, height: 52, borderRadius: 14, fontSize: 22, cursor: "pointer",
                      border: form.energie === n ? "2px solid var(--app-accent)" : "1px solid var(--app-border)",
                      background: form.energie === n ? "var(--app-surface-2)" : "var(--app-surface)", color: "var(--app-text)",
                    }}>
                    {["😴", "😔", "🙂", "😃", "🤩"][n - 1]}
                  </button>
                ))}
              </div>
              <p style={{ textAlign: "center", fontSize: 12, color: "var(--app-text-muted)", marginTop: 8 }}>
                On l'utilisera pour adapter votre charge et vous rappeler de souffler.
              </p>
              <Nav onBack={() => setScreen(12)} onNext={() => setScreen(9)} />
            </>
          )}

          {/* Chapitre — Action : 3 premières tâches */}
          {screen === 14 && (() => {
            const firstActions = [
              `Clarifier votre offre : ${form.projet_description ? form.projet_description.slice(0, 60) : "votre proposition de valeur"}`,
              form.priorites[0] ? `Lever le frein « ${form.priorites[0]} »` : "Identifier votre premier client cible",
              form.objectif_90j ? `Avancer vers : ${form.objectif_90j}` : "Planifier votre première action commerciale",
            ];
            return (
              <>
                <h2 className="ob-title" data-testid="ob-step-title">Vos 3 premières actions</h2>
                <p className="ob-sub">Votre vision devient concrète. L'IA a préparé vos premières actions — vous les retrouverez dans votre cockpit.</p>
                <ul style={{ listStyle: "none", padding: 0, margin: "6px 0", display: "flex", flexDirection: "column", gap: 10 }}>
                  {firstActions.map((a, i) => (
                    <li key={i} data-testid={`ob-action-${i}`} style={{ display: "flex", gap: 10, alignItems: "flex-start", padding: "12px 14px", borderRadius: 12, background: "var(--app-surface)", border: "1px solid var(--app-border)", color: "var(--app-text)" }}>
                      <span style={{ minWidth: 22, height: 22, borderRadius: 999, background: "var(--app-accent)", color: "#0B1F3A", fontWeight: 800, fontSize: 12, display: "inline-flex", alignItems: "center", justifyContent: "center" }}>{i + 1}</span>
                      <span style={{ fontSize: 14, lineHeight: 1.4 }}>{a}</span>
                    </li>
                  ))}
                </ul>
                <Nav onBack={() => setScreen(10)} onNext={() => setScreen(15)} nextLabel="Créer mon 1er document" />
              </>
            );
          })()}

          {/* Chapitre — Premier document */}
          {screen === 15 && (
            <>
              <h2 className="ob-title" data-testid="ob-step-title">Votre premier document est prêt</h2>
              <p className="ob-sub">L'IA a rédigé votre <b>One-Pager Vision</b>. Téléchargez-le — la synchronisation avec votre Drive arrive bientôt.</p>
              <div data-testid="ob-document-preview" style={{ background: "var(--app-surface)", border: "1px solid var(--app-border)", borderRadius: 14, padding: 16, textAlign: "left", color: "var(--app-text)", maxHeight: 200, overflow: "auto" }}>
                <div style={{ fontWeight: 700, marginBottom: 6 }}>🎯 One-Pager Vision — {form.prenom}</div>
                <div style={{ fontSize: 13, whiteSpace: "pre-wrap", lineHeight: 1.5, color: "var(--app-text-muted)" }}>{visionDocText}</div>
              </div>
              <div style={{ display: "flex", gap: 10, marginTop: 14, flexWrap: "wrap", justifyContent: "center" }}>
                <button className="ob-cta" data-testid="ob-document-download" onClick={downloadVisionDoc}><Download size={16} /> Télécharger</button>
                <button className="ob-btn-ghost" data-testid="ob-document-drive" disabled title="Bientôt disponible" style={{ opacity: 0.6, cursor: "not-allowed" }}>Ranger dans mon Drive (bientôt)</button>
              </div>
              <Nav onBack={() => setScreen(14)} onNext={() => setScreen(11)} nextLabel="Terminer" />
            </>
          )}
          ...
          {screen === 10 && (
            <>
              ...
              <ul className="ob-summary" data-testid="ob-summary" style={{ listStyle: "none", margin: 0, padding: 0, display: "flex", flexDirection: "column", gap: 8 }}>
                ...
                <li style={SUMMARY_ROW_STYLE}><b style={SUMMARY_LABEL_STYLE}>Ambiance</b><span style={SUMMARY_VALUE_STYLE}>{form.ambiance === "clarte" ? "Univers Clarté" : form.ambiance === "foi" ? "Univers Foi" : "Univers Sens"}</span></li>
          ...
          {screen === 11 && (
            <>
              ...
              <BusinessIdentityCard
                prenom={form.prenom}
                vision={{
                  why: form.pourquoi || form.objectif_90j || "",

...
import { PLANS_LANCEMENT, prixMois, prixFondateurMois, dateFinFr } from "@/lib/plans";
...
function Carte({ o, cycle, fondateur, embed }) {
  ...
  const chemin = `/login?next=${encodeURIComponent(`/onboarding?plan=${o.key}&cycle=${cycle}`)}`;
  ...
  const libelle = o.mensuel === 0 ? "Commencer gratuitement" : `Choisir ${o.nom}`;
  return (
 ...
 * offres : clés à afficher (par défaut Découverte, Solo, Pro).
...
export default function GrilleTarifs({ embed = false, offres = null, contact = true, cycleInitial = "mensuel" }) {
  ...
  return (
    <div>
      ...
      <p className="mt-6 text-center text-[11.5px] text-offwhite/45">Prix HT · sans engagement ni renouvellement automatique · paiement sécurisé Mollie</p>
    </div>
  );
}

  ...
  return (
    <div className="min-h-screen bg-[#081734] text-white" data-testid="campus-app-root">
      ...
      <aside className="fixed inset-y-0 left-0 z-40 hidden w-[104px] flex-col items-center border-r border-white/10 bg-[#0B1F3A]/70 px-3 py-5 backdrop-blur-xl md:flex" data-testid="campus-sidebar">
        ...
        <div className="mt-3 text-center"><p className="text-[9px] font-bold uppercase tracking-[.18em] text-[#E5C887]">Campus</p><p className="mt-1 text-[9px] leading-tight text-white/45">Pratique & progrès</p></div>
      ...
      <div className="md:pl-[104px]">
        <header className="sticky top-0 z-30 flex min-h-[70px] items-center justify-between gap-3 border-b border-white/10 bg-[#0B1F3A]/35 px-4 py-3 backdrop-blur-xl sm:px-7" data-testid="campus-header">
          <div className="flex min-w-0 items-center gap-3">
            <button onClick={() => navigate("/campus")} className="flex h-9 w-9 items-center justify-center rounded-xl border border-white/15 bg-white/[.06] md:hidden" aria-label="Accueil Campus"><GraduationCap size={18} className="text-[#E5C887]" /></button>
            <div className="min-w-0"><p className="text-[10px] font-semibold uppercase tracking-[.16em] text-[#E5C887]">MyExtension</p><p className="font-head text-base font-semibold text-white">Campus</p></div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <span className="flex h-9 min-w-9 items-center justify-center rounded-xl border border-white/15 bg-white/[.06] px-2 text-xs font-bold text-[#E5C887]" title={profileName || "Mon compte"}>{profileName ? profileName.charAt(0).toUpperCase() : <UserRound size={16} />}</span>
          </div>
        </header>

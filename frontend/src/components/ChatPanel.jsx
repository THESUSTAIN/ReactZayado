...
function DraftMessage({ content }) {
  ...
  return (
    <div className="rounded-xl border border-[#D4AF37]/35 bg-[#0A1128]/40 p-3" data-testid="copilot-draft-message">
      <div className="mb-1.5 flex items-center justify-between gap-2">
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          <a href={mailtoHref} className="inline-flex items-center gap-1 rounded-full bg-[#D4AF37] px-2.5 py-1 text-[11px] font-semibold text-[#0A1128] hover:opacity-90" data-testid="draft-transmit">
...
function ActionCard({ card, onDecision, busy, index = 0 }) {
  ...
  return (
    <div className={`rounded-2xl border p-3.5 transition-all duration-500 ${rank === 1 ? "border-[#D4AF37]/65 bg-[#D4AF37]/[0.09] shadow-[0_0_24px_rgba(212,175,55,0.08)]" : "border-white/30 bg-white/[0.08]"} ${urgent ? "border-rose-400/80 bg-rose-400/10" : ""} ${card.justCreated ? "mission-created-exit" : ""}`} data-testid={`copilot-action-card-${card.source_key}`} data-urgent={urgent ? "true" : "false"} data-priority-rank={rank}>
      <button type="button" className="mb-2 flex w-full items-center gap-2.5 text-left" onClick={() => setExpanded((value) => !value)} aria-expanded={expanded} data-testid={`copilot-action-toggle-${rank}`}>
        <span className={`inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-xl ${rank === 1 ? "bg-[#D4AF37] text-[#0A1128]" : "bg-[#D4AF37]/20 text-[#E8C96A]"}`}><strong className="text-[13px]">{rank}</strong></span>

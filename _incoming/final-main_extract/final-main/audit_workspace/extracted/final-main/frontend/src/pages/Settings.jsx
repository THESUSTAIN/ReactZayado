...
import {
  ...
  Save, ExternalLink, ChevronRight, Upload, Image as ImageIcon,
  Menu, Settings2, Search,
  // ─── icônes utilisées dans l'aide contextuelle (aperçu visuel des boutons) ───
  Plus, MoreHorizontal, Sparkles, Target, FileText, TrendingUp,
  Wallet, AlertTriangle, LineChart, Battery, ShieldAlert, ShoppingBag,
  Radar, Kanban, MessageSquare, Filter, Sliders, GitCompare, ArrowRight,
  Folder, FileUp, StickyNote, Wand2, ArrowLeftRight, MapPin,
  GraduationCap, Send, Users, CheckCircle2, Circle, Map as MapIcon,
  ThumbsUp, Zap,
} from "lucide-react";
        ...
        <div style={{ marginBottom: 14 }} data-testid="inspiration-live-preview">
          <div style={{
            ...
            <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, padding: 14 }}>
              <p style={{ fontFamily: "'Instrument Serif', serif", fontStyle: "italic", color: "#F6F2EA", fontSize: 14, margin: 0 }}>« Aperçu de votre écran d'inspiration »</p>
            </div>
  ...
  return (
    <div className="settings-shell" data-testid="settings-shell">
      ...
      <div className="settings-panel" data-testid={`settings-panel-${active}`}>
        ...
        {active === "profil" && (
          <>
            <div className="glass-card" style={{ marginBottom: 16 }} data-testid="settings-profile">
              <div className="card-label"><User size={14} /> {t("set_profile")}</div>
              <div style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 12 }}>
                <input className="zinput" placeholder={t("set_name")} value={profile.name} onChange={e => setProfile(p => ({ ...p, name: e.target.value }))} onBlur={e => saveProfile({ name: e.target.value })} data-testid="set-profile-name" />
                <input className="zinput" placeholder={t("set_email")} value={profile.email} onChange={e => setProfile(p => ({ ...p, email: e.target.value }))} onBlur={e => saveProfile({ email: e.target.value })} data-testid="set-profile-email" />
              </div>
            </div>
            <MemoireSection />
          </>
...
export function SettingsModal({ open, onClose }) {
  ...
  return (
    <AnimatePresence>
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
        style={{ position: "fixed", inset: 0, background: "rgba(11,31,58,0.68)", backdropFilter: "blur(8px)", zIndex: 900, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}
        onClick={onClose}>
...
// ─── PAGE PARAMÈTRES (route /parametres) ──────────────────────
export default function Settings() {
  const { t } = usePrefs();
  return (
    <motion.div className="page" data-testid="page-settings" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
      <div className="page-head"><div><h1 className="page-title">{t("settings")}</h1></div></div>
      <SettingsContent />
    </motion.div>
  );
}
